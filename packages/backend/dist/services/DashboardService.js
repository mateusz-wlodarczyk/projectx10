...
class DashboardService {
     ...
     */
    async getKeyMetrics(boatType = "catamaran", period = "month") {
        try {
     ...
     */
    async getWeeklyPriceTrends(boatType = "catamaran", year = 2025) {
        try {
            const boats = await this.getAggregatedBoatData(boatType);
     ...
     * Get discount trends
     */
    async getDiscountTrends(boatType = "catamaran", timeRange = "month") {
        try {
            const boats = await this.getAggregatedBoatData(boatType);
            const dataPoints = [];
     ...
     */
    async getAvailabilityTrends(boatType = "catamaran", timeRange = "month") {
        try {
            const boats = await this.getAggregatedBoatData(boatType);
            const dataPoints = [];
     ...
     * Get revenue trends
     */
    async getRevenueTrends(boatType = "catamaran", timeRange = "month") {
        try {
            const boats = await this.getAggregatedBoatData(boatType);
            const dataPoints = [];
     ...
     */
    async getSummaryStats(boatType = "catamaran") {
        try {
            const boats = await this.getAggregatedBoatData(boatType);
            const calculations = await this.calculateDashboardMetrics(boats);
            return [
     ...
     */
    async calculateDashboardMetrics(boats) {
        if (boats.length === 0) {
            return {
                totalBoats: 0,
                averagePrice: 0,
